...
const RadiiInfo = ({ name, boxSize }: RadiiInfoProps) => {
  let pixel;
    ...
    <Flex py="2.5" borderBottom="1px" borderColor="gray.200">
      ...
      <Box>
        <Box
          ...
          w={boxSize}
          h={boxSize}
          border="1px solid var(--chakra-colors-goldn)"
        ></Box>
      </Box>
    </Flex>
  );
...
export const Radii = ({ boxSize = "70px" }: RadiiProps) => {
  return (
    <Box w="100vw">
      <Box>
        ...
        {radii.map((radius, index) => (
          <RadiiInfo key={index} name={radius} boxSize={boxSize} />
        ))}
      </Box>
    </Box>
  );
